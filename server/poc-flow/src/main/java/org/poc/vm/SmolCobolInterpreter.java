...
public class SmolCobolInterpreter implements CobolInterpreter {
    ...
    public SmolCobolInterpreter(StackFrames runtimeStackFrames, ExecuteCondition condition, ConditionResolver conditionResolver, Breakpointer bp) {
    ...
    public CobolInterpreter scope(ChartNode scope) {
        return new SmolCobolInterpreter(runtimeStackFrames.add(scope), condition, conditionResolver, breakpointer);
    }

    @Override
    public CobolVmSignal execute(ChartNode node) {
        return condition.run(() -> {
            return breakpointer.run(node, () -> {
    ...
    @Override
    public CobolVmSignal executePerformProcedure(List<ChartNode> procedures, ChartNodeService nodeService, ChartNode node) {
        return condition.run(() -> {
            return breakpointer.run(node, () -> {
    ...
    @Override
    public CobolVmSignal executeGoto(List<ChartNode> destinationNodes, ChartNodeService nodeService, ChartNode node) {
        return condition.run(() -> {
            return breakpointer.run(node, () -> {
    ...
    @Override
    public CobolVmSignal executeExit(ChartNodeService nodeService, ChartNode node) {
        return condition.run(() -> {
            return breakpointer.run(node, () -> {
...
//        System.out.println(runtimeStackFrames.stackTrace());
    ...
    @Override
    public CobolVmSignal executeNextSentence(ChartNodeService nodeService, ChartNode node) {
        return condition.run(() -> {
            return breakpointer.run(node, () -> {
    ...
    @Override
    public CobolVmSignal executeDisplay(List<CobolParser.DisplayOperandContext> messages, ChartNodeService nodeService, ChartNode node) {
        return condition.run(() -> {
            return breakpointer.run(node, () -> {
    ...
    @Override
    public CobolVmSignal executeMove(ChartNode node, ChartNodeService nodeService) {
        return condition.run(() -> {
            return breakpointer.run(node, () -> {
                MoveChartNode move = (MoveChartNode) node;
    ...
    @Override
    public CobolVmSignal executeAdd(ChartNode node, ChartNodeService nodeService) {
        return condition.run(() -> {
            return breakpointer.run(node, () -> {
                AddChartNode add = (AddChartNode) node;
...
//        return new SmolCobolInterpreter(new ExecuteAtTargetFlipCondition(specificLocation), runtimeStackFrames);
        return CobolInterpreterFactory.interpreter(new ExecuteAtTargetFlipCondition(specificLocation), runtimeStackFrames, conditionResolver, breakpointer);
    }
